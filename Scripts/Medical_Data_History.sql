...
select patient_id, first_name from patients where first_name like "S%" and first_name like "%s" and char_length(first_name) >= 6;
...
select count(*) as total_patients ,city from patients group by city order by total_patients desc,city asc;
...
select patient_id as id, first_name, last_name, 'Patient' AS role
from patients
union all
SELECT doctor_id AS id, first_name, last_name, 'Doctor' AS role
FROM doctors;
...
select day(admission_date) as days_of_month, count(*) as total_admissions from admissions group by day(admission_date) order by total_admissions;
...
select p.patient_id, p.first_name, p.last_name, d.specialty as doctor_specialty from 
patients as p
